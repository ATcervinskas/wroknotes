...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    public function getContent()
    {
        ...
        $query = 'SELECT * FROM `' . _DB_PREFIX_ . 'dpd_pudo_cart` WHERE `id_cart` = ' . $this->order->id_cart;
        $result = $db->executeS($query);

        if (!empty($result)) {
            $pickup_point_street = $result[0]['street'];
            $pickup_point_city = $result[0]['city'];
        } else {
            // Handle the case where the result is empty
            $pickup_point_street = null;
            $pickup_point_city = null;
        }

        $pickup_point = null;
        if (!empty($pickup_point_city) && !empty($pickup_point_street) && strpos($carrier->name,"Pristatymas į paštomatą") !== false){
            $pickup_point_street = ucfirst(strtolower($pickup_point_street));
            $pickup_point_city = ucfirst(strtolower($pickup_point_city));

            if (substr($pickup_point_street, -1) == ' ') {
                $pickup_point_street = rtrim($pickup_point_street);
            }

            $pickup_point = "$pickup_point_street, $pickup_point_city, DPD paštomatas";
        }

        $office_pickup = null;
        if(strpos($carrier->name, "Atsiėmimas biure ") !== false) {
            $office_pickup = "Atsiėmimas biure (Baltosios Vokės g. 62, Juodšiliai, 14101)";
        }
